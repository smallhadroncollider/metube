...
const getMinDuration = (): number => parseInt(process.env.MIN_DURATION ?? "60", 10);
const getMaxDuration = (): number => parseInt(process.env.MAX_DURATION ?? "3600", 10);
...
export const apiRoutes = (db: Database): IRouter => {
	...
	router.post("/videos/:videoId/add", async (req: Request, res: Response) => {
		...
		if (req.session.oauth2Client) {
			...
			if (!result.ok) {
				console.error(`Failed to add video ${videoId} to playlist:`, result.error);
